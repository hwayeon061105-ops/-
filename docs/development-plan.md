...
#### 2) 세부 작업 항목 (Tasks)
- [x] **PRD 완료 조건 검증 (Checklist Validation)**
  - [x] 키, 몸무게, 현재 마음가짐 입력 후 버튼 클릭 시 5~10초 사이에 결과가 표시되는가?
  - [x] 입력 항목이 비어있을 경우 빨간 글씨로 `"00칸이 비어있어요"` 문구가 뜨고 다음 단계로 넘어가지 않는가?
  - [x] 처리 중(5~10초) 동안 조금씩 움직이며 운동하는 캐릭터 애니메이션이 렌더링되는가?
  - [x] 결과 조언에 이모티콘/표정 특수문자가 포함되어 있으며, 정확히 3줄로 출력되는가?
  - [x] AI 응답이 실패하더라도 화면이 멈추지 않고, '다시 하기' 또는 재시도 버튼이 정상 동작하는가?
  - [x] 새로고침 시 초기 입력 페이지로 정상 되돌아가는가?
- [x] **UI/UX & 디바이스 최적화**
  - [x] 모바일, 태블릿, 데스크톱 반응형 뷰포트 정밀 튜닝
  - [x] 스크린 리더 지원(Aria-live, Role, Label, RadioGroup 키보드 탐색) 및 포커스 트랩 방지
- [x] **문서화 및 유지보수 가이드 업데이트**
  - [x] `README.md` 및 `docs/` 내 API/컴포넌트 명세 정리

### 📸 Sprint 5: 소셜 공유(Viral) & 조언 카드 캡처/복사 기능
> **상태:** ✅ 완료 (Completed)

#### 1) 목표
* 생성된 3줄 맞춤형 운동 조언을 원클릭으로 클립보드에 복사하고, 인스타그램/SNS 공유용 고화질(1080x1350) 카드로 다운로드할 수 있는 파이프라인 구축.

#### 2) 세부 작업 항목 (Tasks)
- [x] **토스트 알림 시스템 (`components/ui/Toast.tsx`)**: 복사/저장 완료 피드백 알림
- [x] **공유 & Canvas 렌더러 (`lib/share.ts`)**: 클립보드 API 및 고화질 인스타 카드 렌더링
- [x] **결과 카드 UI 확장 (`components/result/ResultCard.tsx`)**: 원클릭 공유 툴바 배치
...
| :--- | :---: | :---: | :--- |
| **AI 응답 지연/타임아웃 (10초 초과)** | 높음 | 중간 | 다중 모델(gemini-3.7 -> 3.6) 자동 장애 복구 및 스마트 Fallback 엔진 구비 |
| **AI 포맷 불일치 (줄바꿈 오류)** | 보통 | 높음 | 클라이언트/서버 2중 포맷팅 파서로 3줄 정규화 강제 |
| **API Key 노출 위험** | 높음 | 낮음 | 서버 사이드 Route Handler(`/api/advice`)에서만 API 호출 및 `.gitignore` 엄격 차단 |
